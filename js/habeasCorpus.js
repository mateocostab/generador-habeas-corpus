...
document.getElementById('botonGenerar').onclick=function(){
    exportarPDF()
}
    //variables
    //labels
    ...
    if(iGenero==='1')
    {
        ...
        if(iSituacion.value === '1'){
            doc.text('El señor' + iNombre + 'fue aprendido por' + iAutoridadRealizo + 'el pasado' + iFecha + 'en' + iCiudad + 'por orden de' + iAutoridadOrdeno +  'Desde entonces hasta la fecha han transcurrido' + iDiasTrans +', sin que el mismo haya sido indagado o resuelta su situación jurídica.',10,105)
        }
        else{
            doc.text('El señor' + iNombre + 'se encuentra recluido en' + iLugar + ', a partir del día'+ iFecha + 'y el funcionario que ordenó su aprehensión es' + iAutoridadOrdeno + 'quien se desempeña como' + iAutoridadRealizo,10,105)
        }
        doc.addPage()
    ...
    //Caso 2
    else if(iGenero.value ==='2')
        {
            ...
            doc.text("HECHOS",10,95, 'center') 
            if(iSituacion.value === '1'){
                doc.text('La señora' + iNombre + 'fue aprendida por' + iAutoridadRealizo + 'el pasado' + iFecha + 'en' + iCiudad + 'por orden de' + iAutoridadOrdeno +  '. Desde entonces hasta la fecha han transcurrido' + iDiasTrans +', sin que la misma haya sido indagada o resuelta su situación jurídica.',10,105)
            }
            else{
                doc.text('La señora' + iNombre + 'se encuentra recluida en' + iLugar + ', a partir del día'+ iFecha + 'y el funcionario que ordenó su aprehensión es' + iAutoridadOrdeno + 'quien se desempeña como' + iAutoridadRealizo,10,105)
            }
            doc.addPage()
            doc.setFont('Times', 'italics')
            doc.text(p1,10,10)
        ...
        doc.text("Yo,"+ iNombre + ', en mi condición de ' + iCondicionDe +', mayor y vecine de la ciudad de' + iCiudad + ', identificade con cédula de ciudadanía'+ iNumeroCC +'de'+ iCiudadCC + ', por medio del presente escrito acudo ante la autoridad judicial para solicitar se sirva dar trámite a la petición de HÁBEAS CORPUS en favor de ' + iAgredido + ', con fundamento en los siguientes:',20,65)
            ...
            if(iSituacion.value === '1'){
                ...
                doc.text(iNombre + 'se encuentra recluide en' + iLugar + ', a partir del día'+ iFecha + 'y el funcionario que ordenó su aprehensión es' + iAutoridadOrdeno + 'quien se desempeña como' + iAutoridadRealizo,10,105)
            }
            doc.addPage()
            doc.setFont('Times', 'italics')
            doc.text(p1,10,10)
